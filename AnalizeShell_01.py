...
start_date_time_daily = datetime(2023, 3, 13, 12, 0, 0)
# end_date_time_daily = yesterday.replace(hour=23, minute=59, second=0, microsecond=0)
end_date_time_daily = datetime(2023, 3, 15, 12, 0, 0)
...
#============================================================================================================
# crnt_city = 'Achisamach'
# paramName = 'SO2'
# timeWindowStr = 'israel_24hr'
...
#
# thresholdValue = params_air_tbl.loc[params_air_tbl['param'] == paramName][timeWindowStr].values[0]
#
#
# timestamps, values = analyze.moving_average_threshold(df, timeWindow = timeWindowSec, city = crnt_city, param = paramName, threshold = thresholdValue)
    ...
    for crnt_city in pd.unique(df.Id):
        for i_t_daily in range(0,len(t_win_vec_daily)-1):
            ...
            if not(crnt_daily_data.empty):
                ...
                df_daily = pd.concat([df_daily, crnt_df_daily])
                b=1
        for i_t_weekly in range(0,len(t_win_vec_weekly)-1):
            ...
            if not (crnt_weekly_data.empty):
                ...
                df_weekly = pd.concat([df_weekly, crnt_df_weekly])
